...
#-----------------------------------------------------------------------------
vendor_saved_apps := Bluetooth BluetoothMidiService BookmarkProvider EasterEgg \
                     ExtShared HTMLViewer KeyChain PacProcessor PrintRecommendationService \
                     PrintSpooler Stk UserDictionaryProvider WallpaperBackup WAPPushManager \
                     BackupRestoreConfirmation BlockedNumberProvider CarrierConfig CellBroadcastReceiver \
                     DefaultContainerService EmergencyInfo ExternalStorageProvider FusedLocation \
                     InputDevices ManagedProvisioning MtpDocumentsProvider ProxyHandler SharedStorageBackup \
                     Shell StatementService StorageManager WallpaperCropper webview CMSettingsProvider
